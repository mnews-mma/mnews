...
import { getEvent, OG_RESULT_WINNER_NAME_CEILING, OG_RESULT_WINNER_NAME_FLOOR } from "@/lib/events";
import { fitResultWinnerNameSize } from "@/lib/og/resultNameFit";
import {
  OG_COLORS as COLORS,
  SITE_URL,
  loadOgFonts,
...
// 勝者名フォントサイズ: 全RESULTカード共通の単一サイズ(天井)で統一する
// (2026-07-27)。天井はevents.tsのOG_RESULT_WINNER_NAME_CEILING = EVENTS
// (RESULTカードが実際に表示し得る全対戦の勝者候補)の中で最も長い名前が
// カード内寸にちょうど収まるサイズを逆算した値で、ベタ書きしない。
// 天井以下の名前は必ず天井ちょうどで描画され、天井算出の母集団に無い
// 将来の極端な長い名前だけが下限まで自然に縮小する(shrink-to-fit、拡大はしない)。
// 旧実装は「基準サイズ+個別shrink」だったため、短い名前ほど天井に張り付いて
// 見た目上は単一サイズに見えても、天井以下の名前同士でも縮小量がバラつき得た
// (実質は個別width-fitのまま)。天井を「母集団の最も厳しい1名」から機械的に
// 算出することで、天井以下の名前は理論上も全カード完全に同一サイズになる。
//
// 勝者名エリアの内寸(カード幅1200px - 本体左右パディング56px×2)。
// この値はevents.ts側のRESULT_WINNER_MAX_WIDTH_PXと同じ値を独立に持つ
// (events.ts側がOGPルートの実装詳細に依存しないようにするため。変更する
// 場合は両方を揃えること)。
const WINNER_NAME_MAX_WIDTH_PX = 1200 - 56 * 2;
// 縮小時にベースラインがズレて見えないよう、勝者名の描画エリア自体は
// 天井算出の基準サイズ(120px、events.tsのreduce初期値と同じ)を元にした
// 固定高さにして上下中央揃えで安定させる。
const WINNER_NAME_AREA_HEIGHT_PX = Math.round(120 * 1.15);
...
export async function GET(req: Request) {
  try {
    ...
    const winnerSize = fitResultWinnerNameSize(
      winner,
      WINNER_NAME_MAX_WIDTH_PX,
      OG_RESULT_WINNER_NAME_CEILING,
      OG_RESULT_WINNER_NAME_FLOOR
    );
