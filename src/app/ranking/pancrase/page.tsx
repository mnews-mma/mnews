...
export async function generateMetadata() {
  ...
  const prefix = "パンクラス（PANCRASE）公式ランキングを階級別に掲載。";
  const suffix = "の王者・ランカーを最新の公式発表から転載。";
  const fallbackClasses = "フライ級・バンタム級・フェザー級・ライト級";
  const classList = pancrase
    ? buildOfficialRankingWeightClassList(pancrase, RANKING_DESCRIPTION_MAX - fullWidthLength(prefix) - fullWidthLength(suffix))
    : fallbackClasses;
  return pageMetadata({
