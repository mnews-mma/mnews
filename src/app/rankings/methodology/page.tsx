  ...
  return (
    <>
      ...
      <div className="prose">
        ...
        <ul style={{ color: "var(--muted)", lineHeight: 1.9 }}>
          <li>王者・挑戦者を区別せず、レートの絶対値だけで階級を横断して並べます(階級ごとの平均で正規化はしません)。</li>
          <li>王者(RIZIN公式が認定する現王者)は必ず上位に来るとは限りません。タイトル防衛回数は参考情報として表示しますが、順位には使いません。</li>
          <li>
            <strong>階級別ランキングとは順位が食い違うことがあります。</strong>
            階級別ランキングは「その階級の中で誰が強いか」を直接対決の結果を優先して決めますが、P4Pは対戦相手を問わず
            積み上げたレートの絶対値だけで決めます。そのため、同じ階級の中でも直接対決で勝っている選手がP4Pでは
            レートの絶対値で負けている選手より下に来る、といった逆転が起こりえます。これは実装上の不整合ではなく、
            2つのランキングが異なる問いに答えているために生じる、意図的な仕様です。
          </li>
        </ul>
