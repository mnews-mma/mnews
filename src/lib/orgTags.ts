...
//   修斗       … 現ランカーのみ(同上)
//   DEEP       … 2026年1月以降に開催されたDEEP主催イベント(org="deep")出場者のみ
//     (ナンバーシリーズに限らず地方大会・DEEP JEWELSも含む。日付駆動判定)
//   RIZIN      … 2025年1月以降にRIZIN主催のMMA興行に出場した選手のみ
...
export function computeFighterTags(f: TaggableFighter, orgRankings: OrgRankingsFile): OrgTag[] {
  ...
  for (const key of ["pancrase", "shooto"] as const) {
    ...
    for (const c of data.classes) {
      ...
      if (hit) {
        ...
        if (key === "pancrase") pancraseTagged = true;
        break;
      }
    }
  }
  ...
  // パンクラスの明示例外(対象外だが現ランカーと同格に扱うべき選手)。
  if (!pancraseTagged && PANCRASE_TAG_EXCEPTIONS.has(f.slug)) {
