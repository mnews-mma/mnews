// RIZIN王者のタイトル防衛回数。champions.ts(現王者名オーバーレイ)と同じ思想:
// 一次ソースで確認できる「事実」のみを載せ、根拠不明な推測は一切行わない(捏造ゼロ)。
//
// 【現在の利用状況・重要】2026-07-22時点で、このデータはP4Pランキングの
// 順位計算にも画面表示にも使っていない(いずれも人間レビューを経て撤回済み:
// 当初は王者ティア序列のタイブレークに使う設計 → 表示のみの参考情報 → 表示も
// 取りやめ)。データ・取得経路・自己検証(取得不能の明示フラグ)は将来の再利用に
// 備えて意図的に残している(据え置き判断、2026-07-22)。したがって現状このファイル
// を更新しなくても公開ページの表示は一切変わらないが、再び使う判断をしたときに
// 数値が陳腐化していないよう、下記の運用ルールは維持する。
//
// 【運用ルール・厳守】
