...
// カードデザインのバージョン。デザインを変えたら +1 する。デザイン以外でも
// カード内の数値・文言の算出ロジックを変える場合(例: /api/og/fighterが
// 参照する戦績ソースの切り替え)も、古いキャッシュ画像に誤った数値が残り
// 続けるのを防ぐため同様に+1する。
// 全カード種別(個人/MATCH UP/結果/ダイジェスト)共通のキャッシュバスターで、
// ?v= が変わることでCDN・ブラウザ・X側の古い画像キャッシュを無効化する。
// 「カード種別ごとの個別キャッシュ対策」はしない — 必ずこの共通機構を使うこと。
export const OG_DESIGN_VERSION = 18;
...
// 注: VSカードの選手名フィットは src/lib/og/fitName.ts (行分割を事前確定する
// 方式)に置き換え済み。文字幅の概算だけでフォントサイズを決める旧方式
// (fitFontSizeByWidth)は、satoriの自動折り返しに依存し崩れの原因になって
// いたため廃止した。
