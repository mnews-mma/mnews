...
// 「N-M」ペアが2個以上、カンマ/読点/スラッシュ区切りで並び、末尾に任意の
// 丸括弧注記(選手名の投票メモ等)だけが続く行にマッチする。［＊※の接頭辞は
// 許容する(この行形式では※が単なる注記マーカーとして使われるだけで、
// トーナメント勝ち点注記とは行の構造そのものが異なるため)。
// 経緯(2026-08-02、指示書R-7のマージ前確認で発見): 3人のジャッジ採点を
// 「28-30, 29-29, 29-29」のように1行にまとめて記載する書式が一部の大会
// (id=36 猿丸×猿田、id=42 小林(よしずみ)×若林、id=51 齋藤×久保村)に存在し、
// 下のラウンド内訳必須の判定行(1judge1行形式)にもトーナメント注記除外にも
// 該当しないため、単純にジャッジ票が0件のまま握りつぶされ、本来算出できる
// はずの多数決ドロー(majority draw)が「判定不能(unknown)」に落ちていた。
// id=42のケースはノート本文に「2Rは逆に上を奪われドロー」という明示的な
// ドロー記載もあり、単純に読み取れていないだけと裏取り済み。
const PURE_SCORE_LINE =
  /^[※＊]*\s*(\d{1,2}\s*-\s*\d{1,2}(?:\s*[,、/／]\s*\d{1,2}\s*-\s*\d{1,2}){1,})\s*(?:[（(][^）)]*[）)])?\s*$/;

function judgeMajority(noteRaw: string): "A" | "B" | "tie" | null {
  const lines = noteRaw.split("\n").map((l) => l.trim()).filter(Boolean);
  const tallies: Array<[number, number]> = [];
  for (const line of lines) {
    const pureScoreMatch = line.match(PURE_SCORE_LINE);
    if (pureScoreMatch) {
      for (const pair of pureScoreMatch[1].matchAll(/(\d{1,2})\s*-\s*(\d{1,2})/g)) {
        tallies.push([Number(pair[1]), Number(pair[2])]);
      }
      continue;
    }
