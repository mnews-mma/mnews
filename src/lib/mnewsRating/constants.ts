...
// H2H単調性オーバーレイの適用範囲(2026-07-19): この月数より古い直接対決は順位固定に使わない。
// 基準日は壁時計の今日ではなく「データ上の最新bout日(=最新イベント)」に固定する(データ駆動)。
// 新しい試合が入らない限りcutoffが動かず、無風日の再生成で順位がドリフトしない。
export const MONOTONICITY_H2H_RECENCY_MONTHS = 12;
