...
// H2H単調性オーバーレイの適用範囲(2026-07-19導入・月数窓 → 2026-08-13廃止):
// 以前はMONOTONICITY_H2H_RECENCY_MONTHS(12→18ヶ月)という経過月数の機械的な
// 窓で古い直接対決を切り捨てていたが、全階級横断のlatestBoutDateを基準に
// しているため、無関係な階級での1大会の反映だけで窓全体が前進し、別階級の
// 古いH2Hペアが窓外に押し出される副作用があった(実例: 野村駿太×パトリッキー・
// ピットブル2025-07-27戦が、RIZIN.54反映でわずか15日差で窓外に)。
// 「経過月数」ではなく「その後に本人が勝ったという事実」で退役判定する方式
// (monotonicity.tsのretireStaleH2HEdges)に置き換えたため、この定数は廃止。
