...
export const FIGHTER_DIVISION_OVERLAYS: FighterDivisionOverlayEntry[] = [
  ...
  // Gate2パートA(2026-07-20)追加分: 567バケット監査(scripts/audit-bucket-flags-
  // dryrun.ts)のunclassifiable(72〜75kg契約=ウェルター〜ライトヘビー相当、現5階級
  // システムの対象外域)9件のうち、既存のeligibilityScopeStartDate等で既に表示戦績
  // スコープ外だった4件(takeda-koji・izumi-takeshi・ohara-juri・motoya-yukiは
  // 別件で対応済み/非掲載)を除く5選手・6試合を、ケラモフ/トーレスと同種の単発
  // excursionとして個別に除外する。順位・レート・掲載資格には一切影響しない
  // (表示戦績のみ。Eloは階級横断の生涯レートのため変更しない)。
  {
    slug: "case-johnny",
    name: "ジョニー・ケース",
    division: "ライト級",
    recordDisplayExclusions: [{ date: "2022-07-31", opponentSlug: "takeda-koji" }],
    fetchedDate: "2026-07-20",
    note:
      "対武田光司戦(2022-07-31、RIZIN.37)は72.0kg契約=ウェルター相当の単発excursion" +
      "(現5階級システムの対象外域)。前後の試合はいずれも71.0kg契約=ライト級。" +
      "この1敗をライト級としての表示戦績から除外する。除外後の表示戦績は5勝3敗。",
  },
  {
    slug: "yachi-yusuke",
    name: "矢地祐介",
    division: "ライト級",
    recordDisplayExclusions: [{ date: "2017-12-31", opponentSlug: "name:五味隆典" }],
    fetchedDate: "2026-07-20",
    note:
      "対五味隆典戦(2017-12-31、RIZIN FIGHTING WORLD GRAND-PRIX 2017 Final ROUND)は" +
      "72.0kg契約=ウェルター相当の単発excursion(現5階級システムの対象外域、相手は自社DB" +
      "未登録の疑似ノード)。前後の試合はいずれも70.0kg契約=ライト級。この1勝をライト級" +
      "としての表示戦績から除外する。除外後の表示戦績は12勝9敗。",
  },
  {
    slug: "usami-sho-patrick",
    name: "宇佐美正パトリック",
    division: "ライト級",
    recordDisplayExclusions: [{ date: "2022-10-23", opponentSlug: "name:佐々木信治" }],
    fetchedDate: "2026-07-20",
    note:
      "対佐々木信治戦(2022-10-23、RIZIN.39)は75.0kg契約=ミドル相当の単発excursion" +
      "(現5階級システムの対象外域、相手は自社DB未登録の疑似ノード)。直後の試合は71.0kg契約=" +
      "ライト級。この1勝をライト級としての表示戦績から除外する。除外後の表示戦績は4勝3敗。",
  },
  {
    slug: "beinoa",
    name: "ブラックパンサーベイノア",
    division: "ライト級",
    recordDisplayExclusions: [
      { date: "2021-11-20", opponentSlug: "name:ロククダリ" },
      { date: "2025-12-31", opponentSlug: "name:雑賀“ヤン坊”達也" },
    ],
    fetchedDate: "2026-07-20",
    note:
      "対ロクク・ダリ戦(2021-11-20、RIZIN.32、72.0kg契約)・対雑賀“ヤン坊”達也戦" +
      "(2025-12-31、RIZIN 師走の超強者祭り、73.0kg契約)はいずれもウェルター相当の単発" +
      "excursion(現5階級システムの対象外域、相手は自社DB未登録の疑似ノード)。前後の試合は" +
      "71.0kg契約=ライト級で一貫。この2試合(1勝1敗)をライト級としての表示戦績から除外する。" +
      "除外後の表示戦績は3勝3敗。",
  },
  {
    slug: "saiga-yanbo-tatsuya",
    name: "雑賀ヤン坊達也",
    division: "ライト級",
    // 相手表記「“ブラックパンサー”ベイノア」は鍵括弧付きでnorm()(空白・中黒のみ除去)
    // では"beinoa"(nameJa="ブラックパンサーベイノア"、aliases=["ベイノア"])に解決
    // できず、beinoa視点の対戦相手表記(疑似ノード"name:雑賀“ヤン坊”達也")とも別の
    // 疑似ノードになる(buildBoutsのdedupeが効かない非対称な表記揺れ)。そのため
    // opponentSlugはDB内slugではなく実際の疑似ノードID"name:“ブラックパンサー”ベイノア"
    // を指定する(2026-07-20、findZeroMatchRecordDisplayExclusionsで検出・修正)。
    recordDisplayExclusions: [{ date: "2025-12-31", opponentSlug: "name:“ブラックパンサー”ベイノア" }],
    fetchedDate: "2026-07-20",
    note:
      "対ベイノア戦(2025-12-31、RIZIN 師走の超強者祭り)は73.0kg契約=ウェルター相当の単発" +
      "excursion(現5階級システムの対象外域)。前後の試合はいずれも71.0kg契約=ライト級。" +
      "この1勝をライト級としての表示戦績から除外する。除外後の表示戦績は0勝4敗。",
  },
];
