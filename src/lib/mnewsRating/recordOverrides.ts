...
export const RECORD_OVERRIDES: RecordOverride[] = [
  ...
  {
    // 2026-08-04(PR#431の日付ガードが検出): 選手ページの対戦テーブルから
    // /results/lemino-shooto-3 へのリンクが「大会名は一致するのに開催日が
    // 10日ずれている」として落ちていた。上流(Wikipedia戦績表)の試合日が誤り。
    // DEEP 86 IMPACT群と同じ日付誤りパターン(大会名自体は誤りなし)。
    type: "patch-date",
    fighterId: "uchida-takeru",
    date: "2026-02-28",
    opponent: "青井真司",
    correctedDate: "2026-02-18",
    source: "https://www.shooto-mma.com/result/?id=260",
    fetchedDate: "2026-08-04",
    note:
      "修斗公式サイトの「Lemino修斗.3」大会結果ページで確認: 開催日は2026-02-18" +
      "(Wikipedia戦績表は2026-02-28と10日誤記)、会場は後楽園ホール。バンタム級、" +
      "青井心 vs 内田タケル(内田タケルが勝利、1R 1:28 腕十字固め)が実在し、" +
      "決着方法・ラウンド・時間がWikipedia戦績表の記載(1R 1:28 腕ひしぎ十字固め)と" +
      "完全一致するため同一試合と特定できる。大会名(Lemino修斗.3)自体は誤りなし。" +
      "なお対戦相手名も公式は「青井心」でWikipedia戦績表の「青井真司」と異なるが、" +
      "本オーバーライドでは日付のみ訂正し相手名は変更しない(remove+addで消して" +
      "足し直すと訂正の由来が追えなくなるため。相手名だけを訂正する型が無いこと" +
      "自体は別途の課題として監査レポートに記録した)。",
  },
];
