...
for slug, rec in fighter_records.items():
    ...
    for idx, h in enumerate(history):
        ...
        for org in orgs:
            ...
            if result == "nc" and result_type in ("decisive", "draw"):
                b_type.append({
                    "slug": slug, "name": slug_to_name.get(slug, ""),
                    "history_index": idx, "date": date, "event": event_name,
                    "opponent": opponent, "result": result, "org": org,
                    "event_matched": ev["eventName"], "opp_in_org": opp_name_in_bout,
                    "winner_in_org": (winner_name if result_type == "decisive" else "draw(引き分け)"),
                    "expected_result": ("win" if (winner_slug == slug if winner_slug else names_match(winner_name, my_name_in_bout)) else "loss") if result_type == "decisive" else "draw",
                })
            elif result_type == "decisive":
                if winner_slug:
                ...
                if result != "nc":
                    b_type.append({
                        ...
                        "opponent": opponent, "result": result, "org": org,
                        "event_matched": ev["eventName"], "opp_in_org": opp_name_in_bout,
                        "winner_in_org": "nc(無効試合/org側)", "expected_result": "nc",
                    })
