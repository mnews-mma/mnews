...
PR#582マージ直後(本PR着手前)と本PR完了後のkick:data集計値の比較:

| 指標 | 着手前 | 完了後 | 差分 |
|---|---:|---:|---:|
| boutRows(重複除去後) | 32,669 | 32,675 | +6 |
| boutRowsRaw(重複除去前) | 35,207 | 35,231 | +24 |
| boutRowsOfficial | 20,857 | 20,881 | +24 |
| boutRowsWikipedia | 11,812 | 11,794 | −18 |
| mergedDuplicateRows | 2,336 | 2,354 | +18 |
| resultUnknownCount | 134 | 133 | −1 |
| unmatchedBouts | 12 | 12 | 0 |

**内訳の説明(残余ゼロ)**: ⑦で追加した6選手・24件の公式raw行のうち、18件は
既存のWikipedia出典行と同一試合(同日付+同相手)としてdedupeされ、出典が公式へ
昇格しただけで新規の試合行にはならなかった(Wikipedia側の対応行が18件分
alsoFromへ吸収され、boutRowsWikipediaが同数の18減少・mergedDuplicateRowsが
同数の18増加という完全な1:1対応)。残る6件のみが、それまでmnewsのデータに
存在しなかった真に新規の試合行としてboutRowsに反映された(+6)。
resultUnknownCountの−1は⑩で修正したNo Contest不具合の是正分(デニス・ピューリック×
エリアス・マムーディ戦がunknownからno_contestへ訂正されたことによる、選手ごとの
視点違いによる二重カウントを畳んだ後の純減)。unmatchedBoutsは不変であり、
今回の変更が既存の未解決bout集合に影響していないことを裏付ける。

### ⑥への申し送り事項(今回は未着手・未確認のまま記録のみ)

⑩で発見したNo Contest(`is-muted`)未対応の不具合修正は`ingest_one.py`(ONEソース)
限定で行った。**同種の「is-muted相当のCSSクラス・特殊な決着表示パターンが、
⑥で報告した他13ソースのパーサ(ingest_bigbang.py・ingest_deepkick.py等)にも
存在するかどうかは未確認**。各ソースのサイト構造はそれぞれ異なるため同じ不具合が
あるとは限らないが、確認自体を行っていない。今回のスコープでは着手せず、⑥の
13ソース側トラックの調査項目として記録するに留める。

**④・⑩の完了をもってONEトラックはここで終了する。新しいブランチは生やさない。**
⑥で報告した13ソース・22,144件(上記のNo Contest未確認事項を含む)は別トラックとして
扱い、本PRでは着手しない。
