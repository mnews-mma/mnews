...
### 現状のベースライン(2026-08-17、#563マージ後・修正版)

| 区分 | 人数 |
|---|---:|
| スナップショット対象 | 718人 |
| 　└ 戦績表なし(`total=0`、比較対象外) | 216人 |
| 　└ 名前解決不能(比較対象外、下記参照) | 6人 |
| 　└ 比較実施 | 496人 |
| 　　　└ 一致(掲載数が外部基準以上) | 337人 |
| 　　　└ **差分あり(掲載数が外部基準を下回る、ベースライン)** | **159人** |

**名前解決不能6人の内訳**: `アマラ忍`・`クンタップ・ウィラサクレック`・
`ゲーオ・フェアテックス`・`ジョムトーン・チュワタナ`・`ブアカーオ・ポー.プラムック`(いずれも
候補0件)、`龍聖`(候補2件、同名異人)。うち`アマラ忍`はスナップショット取得時点(2026-08-16)
より後にマージされた#563で表記名が「忍アマラー」へ改名反映されたため、旧スナップショットの
表記と一致しなくなったもの(#563のコミットメッセージにある「LEG3_NAME_RENAMES」の対象者)。
`龍聖`の同名異人2件は#563自身の調査でも「既存のDISAMBIGUATION_OVERRIDES機構で正しく1名に
紐付け済み」と報告されており、本ゲートの安全側の設計(2件以上は解決しない)どおりの想定内の
除外。

**差分あり159人という数字は初回実装(6人)から大幅に増えているが、これは想定どおり**
...
**833人への拡張分(+115人、実測では改名5人の新旧名分を含め120人の差分)を本ゲートの
基準に含めるべきか判断した結果: 含めない(159人のまま据え置く)。理由:**
...
4. **未対応のまま残る範囲であることは明示する**: `scripts/check-kick-coverage-gap.ts`は
   ...
   (ゲート判定には使わない)。この120人(≒#563拡張分115人+改名5人の新旧名差分)に対する
   `kana-leg4`相当の独立再抽出は、本PRのスコープ外の次PRへの申し送り事項とする。

### ゲート破壊テストの結果(修正版で再実施)

比較実施グループの中で従来「一致」側だった選手(`aotsu-junpei`(青津潤平)、外部基準27試合)
の掲載数(`index.json`の`boutCount`)を意図的に0へ破壊して実行 → **差分あり件数が基準の
159人から160人へ増加したことを検知しビルド失敗(exit 1)を確認**。`build-kick-data.ts`
を再実行して復元後、OK(159人)に復帰することも確認した。
