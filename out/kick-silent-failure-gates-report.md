...
作成日: 2026-08-17(項目4追補・PR #566対応を反映して更新)
ブランチ: `feat/kick-silent-failure-gates`(PR #564、マージ済み) → 追補
`feat/kick-item4-coverage-gate`(PR #566)

**本PR(および元のPR-G)はゲート(検査スクリプト)の追加と、その検査が現状どれだけの
違反件数を検出するかの可視化のみを行う。`data/kick/*.json` の中身(生データ)は
一切変更していない(新規のベースライン記録用/スナップショットJSONの追加のみ)。**

**PR #566(このセクション以降で追記した「8. 団体公式プロフィール突合」「未対応の残件」)
は、PR-G(#564、マージ済み)とは別の新規PRとして、`kick-item4-coverage-gate`worktreeで
作業した。**
...
## 8. 団体公式プロフィール突合(2つ目のカバレッジ基準、PR #566)

`feat/kick-item4-coverage-gate`(PR #566)で追加。PR-Gの`check-kick-coverage-gap.ts`
(Wikipedia記事の戦績表、独立再抽出718人)に加え、**団体公式サイトのプロフィールページの
戦績サマリー値**を2つ目の独立した外部基準として突き合わせる。

### 位置づけ(必読)

出典調査(`kick-item4-remeasure-current-data.md`、別セッション作成)の結論: 欠落候補
1,464人の大半は公式サイト側の構造(生涯通算サマリー欄と当該団体の個別列挙欄の乖離・
多団体所属者の合算差)で説明でき、無作為抽出した日本人選手30人の個別確認では、単一団体
所属17人は全員「構造的・欠落ではない」、多団体所属13人は要保留のまま未確定、**真の欠落
(パイプラインの取りこぼし)は今回のサンプルでは検出されなかった**。

**このゲートは「真の欠落をゼロにする」ためのものではない。** 現状値をベースラインとして
固定し、今後の悪化(パイプライン側の新規リグレッション)を検知するためのもの。この点は
`scripts/check-kick-official-profile-coverage.ts`冒頭コメントにも明記した。

### 設計

- 外部基準: `data/kick/kickOfficialProfileSnapshot.json`(2,500人、`name`/
  `officialTotal`/`domain`のみ。出典は`kick-official-profile-crosscheck-diffs-current.json`
  の`official_total`列)。取り込みパイプラインを経由しない取得値として凍結する。
- 掲載数: スナップショットに凍結せず、**ビルドのたびに`data/kick/generated/index.json`
  から都度ライブ計算する**(Wikipedia基準の`check-kick-coverage-gap.ts`と同じ設計思想)。
- 選手名→slugの解決: `check-kick-coverage-gap.ts`と同じ`normalizeKickName()` +
  `data/kick/manualOverrides.json`の改名対応表を再利用(前回の反省を踏まえ、改名5人が
  この基準でも名前解決不能にならないよう対応表を通す設計にした。実際にはこの2,500人の
  データセットに改名5人は含まれていなかった)。

### ベースライン: ユーザー提示値との比較

| 区分 | ユーザー提示値(参考測定) | 本ゲートの実測値 | 差 |
|---|---:|---:|---:|
| 一致(diff=0) | 350人 | **349人** | -1 |
| 欠落候補(公式>掲載) | 1,464人・差合計34,940 | **1,452人・差合計34,817** | -12人・-123 |
| 超過(掲載>公式) | 686人・差合計-3,483 | **682人・差合計-3,465** | -4人・+18 |
| 名前解決不能 | 0人(前提上) | **17人** | +17 |

**差の理由**: ユーザー提示値は2,500人全員を比較対象にしているのに対し、本ゲートの実装は
`check-kick-coverage-gap.ts`と同じ安全側の設計(表記名が同一の候補が2件以上=同名異人の
場合は解決しない)を踏襲したため、**17人が名前解決不能**として比較対象から除外された。
この17人の内訳は`龍翔`(4件)・`武蔵`(3件)・`空龍`(2件)・`KAI`(2件)・`大輝`(2件)・
`悠斗`(2件)・`璃久`(2件)・`一輝`(2件)・`龍聖`(2件)の同名異人グループ(#563自身の調査でも
「9グループ・21レコード」と報告されている既知の重複名)で、スナップショット側に同名が
複数回出現する形で計17件が該当した。1(一致)+12(欠落候補)+4(超過)=17と、除外数と
過不足の合計が一致することを確認済み。**架空の数値に寄せず、自分の実装が実際に出した
実測値(欠落候補1,452人・差合計34,817)をそのままベースラインとして採用した。**

`data/kick/kickOfficialProfileCoverageBaseline.json`にratchet方式で記録する。

### ratchet条件

以下の**いずれか**が前回ビルド時点の基準を超えたらビルド失敗:
1. 欠落候補(公式基準 > 掲載数)の人数
2. 欠落候補の差の合計(Σ(official_total - prod_total))

超過側(掲載数 > 公式基準)はratchet対象に含めない(参考値としてログ出力のみ。掲載数が
公式基準を上回るのは多団体所属選手の合算等が主因で、悪化として扱う性質のものではない
ため)。

### ゲート破壊テストの結果

比較実施グループの中で従来「一致」側だった選手(`rika-aimi`(相見梨夏)、公式基準5試合)
の掲載数(`index.json`の`boutCount`)を意図的に0へ破壊して実行 → **欠落候補の人数
(1,452→1,453人)・差の合計(34,817→34,822)の両方が同時に基準超過したことを検知し
ビルド失敗(exit 1)を確認**。`build-kick-data.ts`を再実行して復元後、OK(1,452人・
34,817)に復帰することも確認した。

### このゲートがカバーしていない範囲(残件)

- **名前解決不能17人**: 同名異人(龍翔・武蔵等)。安全側の設計上、意図的に比較対象外。
- **多団体所属者(サンプル30人中13人が該当)**: 出典調査で「要保留」とされた層。
  productionの掲載数は複数団体の合算値である一方、公式サイト側の戦績サマリーは通常
  1団体分のみのため、単純な数値比較では過小/過大に出うる。この層について「真の欠落」の
  有無を確定するには、選手ごとに全所属団体のページを横断確認する必要があり、本PRでは
  未実施(出典調査でも未着手と明記されている)。
- **1,464人 → 実測1,452人という数字自体は「真の欠落の人数」ではない**: 上記の構造的な
  乖離(サマリー欄 vs 個別列挙欄)を含んだ粗い差分であり、出典調査の結論どおり大半は
  パイプラインの欠陥ではない。このゲートの目的は「この粗い差分が今後悪化しないか」の
  監視であって、「真の欠落」の精密な特定ではない。

---

## 未対応の残件(2026-08-17時点、一覧)

PR-Gおよびその追補(本PR #566)のカバレッジ系ゲートが**カバーしていない範囲**を、
見落とされないようここに一覧化する。

| # | 残件 | 対象範囲 | 現状 |
|---|---|---|---|
| 1 | Wikipedia基準(`check-kick-coverage-gap.ts`)が718人固定で、#563が拡張した母集団(833人)のうち約120人分(改名5人の新旧名差分含む)を独立再抽出できていない | 833人母集団のうち約120人 | 独立再抽出(`kana-leg4`方式)が未実施。実行のたびに診断ログ「参考(ゲート対象外): …120人…」を出力(ゲート判定には不使用)。次PRへの申し送り事項としてtask化済み(task_13e2d376) |
| 2 | 公式プロフィール基準(`check-kick-official-profile-coverage.ts`)が同名異人17人を比較対象外にしている | 2,500人スナップショットのうち17人 | 安全側の設計として意図的に除外。同名異人の判別ロジック(所属・生年月日等での一意化)を追加すれば解決可能だが本PRでは未実施 |
| 3 | 公式プロフィール基準が多団体所属者(サンプル30人中13人、全体の約43%相当)の「真の欠落」有無を未確定のまま残している | 日本人選手754人のうち多団体所属者(推定300人強) | 出典調査(`kick-item4-remeasure-current-data.md`)で「要保留」と明記。全所属団体ページの横断確認が必要だが未着手 |
| 4 | サンチャイ・TEPPENGYM選手の対戦相手表示が`splitOpponentGymSuffix`で誤分割される | 表示のみ(戦績データ自体は正しい) | PR-G調査中に発見、別タスクとしてflag済み(task_23f41ec7)。未修正 |

いずれも「ゼロ件ゲート」ではなく、現状値をベースラインとした悪化検知にとどまる。
`data/kick/*.json`の生データ修正・独立再抽出の拡張は、いずれも本PR・PR-Gのスコープ外
(別タスクとして扱う)。
...
## PR #566(項目4追補)での他PR競合・origin/main進行の再確認

作業完了間際に以下を再実行した:

- `git fetch origin main`: origin/mainの新規進行は無し(`feat/kick-item4-coverage-gate`は
  既に最新のorigin/main、commit `50e5082`を含んでいた。merge-base確認で差分0件)。
- `gh pr list --state open`: 本PR(`#566`)以外は`#542`・`#530`・`#524`・`#522`(いずれも
  読み取り専用の調査PR、対象ファイルへの書き込み無し)のみ。`#565`は既にマージ済みで
  リストから消えている。`kick-item4-remeasure`(参照データの取得元)は独自のopen PRを
  持たず、`out/`配下の測定結果のみを提供する非競合のブランチ。

**結論: PR #566作業完了時点でファイル競合・origin/main進行のいずれも無い。**

## PR #566のnpm run build成否

`npm run build`をこのworktree(`kick-item4-coverage-gate`)でフルパス実行し、
新設の`check:kick-official-profile-coverage`ゲート(全13件の`check:kick-*`/
`test:kick-*`の1つ)を含め**全工程が成功することを確認した**。
