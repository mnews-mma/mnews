...
| `update-kick-data.yml` | GHA schedule | `0 19 * * 3` | 木曜4:00 | `data/kick/*`(PR経由、自動マージなし) | 2026-08-21追加。立ち技(/kick)13ソース(K-1/RISE/SHOOT BOXING/KNOCK OUT/Bigbang/Stand up/KROSS×OVER/SNKA/JKA/HoostCup/NJKF/NKB/DEEP☆KICK)を週次取得しゲートが通ればPRを作成(自動マージしない)。RIZIN・Wikipedia・NKB旧サイトは凍結(対象外)。ゲート失敗時はPRを作らずIssue(label: `kick-batch-gate-blocked`)を立てる。他バッチとは自身の作業ブランチにのみpushするためpush競合リスクは無い |
