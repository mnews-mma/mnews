...
| `update-kick-data.yml` | GHA schedule | `0 19 * * 3` | 木曜4:00(初回発火は2026-08-27 4:00 JST) | `data/kick/*`(PR経由、自動マージなし) | 2026-08-21新設・2026-08-22にONE Championshipを追加し計14ソース(K-1/RISE/SHOOT BOXING/KNOCK OUT/Bigbang/Stand up/KROSS×OVER/SNKA/JKA/HoostCup/NJKF/NKB/DEEP☆KICK/ONE)を週次取得しゲートが通ればPRを作成(自動マージしない)。RIZIN・Wikipedia・NKB旧サイトは凍結(対象外)。ONEも選手名簿(one_official_manifest.json、116人固定)自体は拡張しない。ゲート失敗時はPRを作らずIssue(label: `kick-batch-gate-blocked`)、`gh pr create`自体の失敗時も別Issue(label: `kick-batch-pr-blocked`)を立てる。他バッチとは自身の作業ブランチにのみpushするためpush競合リスクは無い。**緊急停止**: `gh workflow disable update-kick-data.yml`(コード変更不要、即時停止。再開は`gh workflow enable update-kick-data.yml`) |
