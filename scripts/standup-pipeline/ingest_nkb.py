...
"""NKB(日本キックボクシング連盟)のboutを抽出する。SCHEMA.mdに準拠。
   ...
   V-2(2026-08、見送り確定・9件): 新サイト90記事をWordPress REST APIで再取得し実測
   (詳細はout/kick-njkf-nkb-standup-deferred-templates.md参照)。
   - 7件は`mainbox_2`マーカー自体が存在しない旧世代テンプレート(2019〜2020年、
     「交戦シリーズ」「出陣シリーズ」各回。slug: 20191012, 20191214-2, 20200208,
     2020620, 20201010, 20201115, 20201212)。※同期間の"pamph"接尾辞記事(前売り券
     告知、試合結果を含まない)38件は元々対象外で、この7件とは別物。
   - 2件(slug: 20240728〈2024年7月28日〉, 20260802〈2026年8月2日〉、いずれも
     「NKジム／テツジム2部主催興行」)は`mainbox_2`があり42ブロック抽出できるが、
     bout行が0件。parse_new_side()が「末尾から2行目=選手名、末尾1行目=所属」と
     仮定しているのに対し、この2記事のテンプレートには「◯戦◯勝◯敗(◯KO)」という
     通算成績行が追加で入っており、名前がその手前にズレる。
   いずれも1件・2件ごとに異なるテンプレート差分で、再利用の効かない一点物の
   パースロジックが必要なため実装を見送る。"""
