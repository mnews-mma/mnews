# -*- coding: utf-8 -*-
"""選手名簿(fighters.json/fighters.csv/README.txt)の生成ロジック。
2026-08-21、build.pyから分離した(週次自動更新ジョブでは呼ばない: 下記参照)。

このファイルはbuild.pyから`import generate_roster`として実行される。build.py側は
`SKIP_FROZEN_SOURCES`が立っている間、このimport自体を行わない(=このファイルの
コードは一切実行されない)。分離した理由:

週次自動更新ジョブ(.github/workflows/update-kick-data.yml)では名簿を再生成せず、
コミット済みのfighters.json/fighters.csvをそのまま入力として使う(=凍結、
RIZIN・Wikipedia・NKB旧サイトと同じ扱い)。2026-08-21の実測で、このファイルが
生成する名簿とコミット済みfighters.jsonの間に86件の差分(うち50件がWikipedia由来の
kana(読み仮名)の消失、5件が選手レコードの分裂、他sources/aliases/orgs/gym)が
あることを確認した。原因はcache/配下の名簿系派生JSON(生成手段が無い、2026-08-18
時点のスナップショット)が、現在コミット済みのfighters.jsonを実際に生成した時点の
入力と完全には一致しないためで、詳細はscripts/standup-pipeline/RAW_CACHE.md参照。
「名簿の自動拡張はしない」がこのジョブのスコープ外事項であるため、そもそも
名簿を再生成する必要が無いという判断で、生成そのものを止める形にした。

このファイル自体(名簿生成ロジック)は変更していない。将来、人間が判断して
cache/配下を更新し名簿を作り直す場合は、このファイルをbuild.py側から
`SKIP_FROZEN_SOURCES=0`(既定)で呼び出せば従来通り動作する。
"""
import json,re,unicodedata,collections,os

CACHE_DIR = 'cache'  # build.py冒頭のCACHE_DIRと値を同期させること(定数複製、循環import回避のため)
...
WPK=json.load(open(f'{CACHE_DIR}/wp_kana.json'))          # leg 5: ja.wikipedia article lead sentences
ROM=json.load(open(f'{CACHE_DIR}/kana_from_romaji.json'))  # leg 4': official romaji, unambiguous only
rows=[]
def add(**kw):
...
RISE_DOB=json.load(open(f'{CACHE_DIR}/rise_dob.json')) if os.path.exists(f'{CACHE_DIR}/rise_dob.json') else {}
SB_DOB=json.load(open(f'{CACHE_DIR}/sb_dob.json')) if os.path.exists(f'{CACHE_DIR}/sb_dob.json') else {}
KO_DOB=json.load(open(f'{CACHE_DIR}/ko_dob.json')) if os.path.exists(f'{CACHE_DIR}/ko_dob.json') else {}

for r in json.load(open(f'{CACHE_DIR}/wp_parsed.json')):
    t=r['wiki_target']
...
BR=json.load(open(f'{CACHE_DIR}/k1_brands.json'))
for r in json.load(open(f'{CACHE_DIR}/k1_parsed.json')):
...
for r in json.load(open(f'{CACHE_DIR}/rise_parsed.json')):
...
for r in json.load(open(f'{CACHE_DIR}/sb_parsed.json')):
...
SKIP=0
for r in json.load(open(f'{CACHE_DIR}/ko_parsed.json')):
...
if os.path.exists(f'{CACHE_DIR}/k1_delisted_merges.json'):
    k1_merges = json.load(open(f'{CACHE_DIR}/k1_delisted_merges.json'))
...
if os.path.exists(f'{CACHE_DIR}/ko_delisted_merges.json'):
    ko_merges = json.load(open(f'{CACHE_DIR}/ko_delisted_merges.json'))
    by_sources_key = {tuple(sorted(r['sources'])): r for r in out}
    ko_merge_applied = 0
    for m in ko_merges:
        target = by_sources_key.get(tuple(sorted(m['existing_sources'])))
...
# ================= CSV for hand-off =================
import csv
ROMSRC=[('k1',f'{CACHE_DIR}/k1_parsed.json'),('knockout',f'{CACHE_DIR}/ko_parsed.json'),('rise',f'{CACHE_DIR}/rise_parsed.json')]
_rom={}
