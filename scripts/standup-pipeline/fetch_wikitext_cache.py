...
raw/wp_wikitext_509.json にキャッシュする(一回限りの取得。ingest_wikipedia.pyは
...
def main():
    ...
    for i in range(0, len(titles), BATCH):
        ...
        time.sleep(0.4)
    json.dump(wikitexts, open("raw/wp_wikitext_509.json", "w"), ensure_ascii=False)
    print(f"完了: {len(wikitexts)}/{len(titles)} 件を raw/wp_wikitext_509.json に保存")
