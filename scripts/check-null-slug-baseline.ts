...
//
// 経緯: #294(DEEP取りこぼし17大会+疑い6大会の取り込み)と#302(指示書I、
// kanru/tenyaのalias反映バックフィル)が互いを知らない状態で独立に
// mainから分岐し、#294が先にマージされたことで#302マージ後の本番ビルドが
// 一時的に失敗した(#310で緊急調整、パンクラス8498・DEEP 3806に設定)。
// 本PR(指示書N、引用符付きニックネーム挿入部の除去マッチ追加)はさらに
// DEEPを1件(hibino-junya)解決したため、DEEPのベースラインを3806→3805に
// 更新する。
const BASELINE = {
  rizin: 1103,
  shooto: 2921,
